...
export class Forms {
    ...
    async openFormsPage() {
        ...
        await this.page.getByRole('link',{name: ' Fill and submit forms with validation scenarios'}).click();
        await expect(this.page).toHaveURL('/practice/forms');
    }

    async fillFirstName(firstName: string) {
        await this.page.getByTestId('input-first-name').fill(firstName);
        await expect(this.page.getByTestId('input-first-name')).toHaveValue(firstName);
    }

    async fillLastName(lastName: string) {
        await this.page.getByTestId('input-last-name').fill(lastName);
    ...
    async selectCountry(country: string) {
        ...
        await expect(this.page.getByTestId('select-country')).toContainText(country);
    }
    ...
    async checkInterest(interest:'selenium' | 'cypress' | 'playwright' | "appium" | 'jest') {
        await this.page.getByTestId(`checkbox-interest-${interest}`).check();
        await expect(this.page.getByTestId(`checkbox-interest-${interest}`)).toBeChecked();
    }
    ...
    async fillConfirmPassword(confirmPassword: string) {
        ...
        await expect(this.page.getByTestId('input-confirm-password')).toHaveValue(confirmPassword);
    }
    ...
    async verifySuccessMessage() {
        await expect(this.page.getByTestId('success-icon')).toHaveText('Form submitted successfully!');
    }
